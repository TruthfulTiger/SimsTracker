...
class UploadController extends Controller {
    ...
    function imageUpload($file,$maxsize) {
        ...
        if ($file) {
            ...
            && $imageFileType != "gif" ) {
            $this->msgicon = 'error';
            $this->uploadResult = 'Sorry, only JPG, JPEG, PNG & GIF files are allowed.';
            $uploadOk = 0;
            }
            ...
            if ($uploadOk == 1) {
                if (move_uploaded_file($tempFile, $targetFile)) {
                    $this->msgicon = 'success';
                    $this->uploadResult = "The file ". htmlspecialchars($filename). " has been uploaded.";
                } else {
                    $this->msgicon = 'error';
                    $this->uploadResult = 'Sorry, there was an error uploading your file.';
                }
            } else {
            $this->msgicon = 'error';
        }
