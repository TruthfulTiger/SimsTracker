<header>
					...
					<li class="nav-item dropdown">
						...
						<div class="dropdown-menu bg-dark">
							<?php foreach (($generators?:[]) as $genkey=>$genitem): ?>
								<a class="dropdown-item text-light" href="<?= ($BASE) ?><?= ($genkey) ?>"><?= ($genitem) ?></a>
							<?php endforeach; ?>
						</div>
					</li>
				</ul>
								...
								<div class="dropdown-menu bg-dark">
									...
									<?php if ($SESSION['user'][1] == 'Admin'): ?>
										...
										<?php foreach (($admin?:[]) as $adminkey=>$adminitem): ?>
											<a class="dropdown-item text-light" href="<?= ($BASE) ?><?= ($adminkey) ?>"><?= ($adminitem) ?></a>
										<?php endforeach; ?>
									<?php endif; ?>
								</div>
							</li>
	...
	<?php if ($SESSION['user']): ?>
		<?php echo $this->render('usermenu.html',NULL,get_defined_vars(),0); ?>
	<?php endif; ?>
</header>
