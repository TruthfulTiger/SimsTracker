<h3 class="text-primary"><?= ($cat) ?> <img src="<?= ($BASE) ?><?= ($s2path) ?>Pets.png" alt="Needs Pets"> <a href="#" class="badge badge-pill badge-secondary title" data-toggle="collapse" data-target="#petsHelp" aria-expanded="false" aria-controls="petsHelp">
	?</a>
	</a></h3>
    ...
    <div class="col">
        <div class="form-group">
            <div class="custom-control custom-checkbox">
                <input type="checkbox" class="custom-control-input" value="1" onchange="petsCheck()" name="casfounder" id="casfounder" <?php if ($s2legacy['casfounder'] == 1): ?>
                checked
                <?php endif; ?>>
        ...
        <div class="form-group">
            ...
            <input type="number" min="0" class="form-control" oninput="adoptedChange(value)" id="adopted" value="<?= (trim($s2legacy['adopted'])) ?>" name="adopted"/>
        </div>
        <div class="form-group">
            <label for="other">Mates brought in by other means</label>
            <input type="number" min="0" class="form-control" id="other" oninput="otherChange(value)" value="<?= (trim($s2legacy['other'])) ?>" name="other"/>
        </div>
        <div class="form-group">
            <label for="topbilling">Heirs that have learned all skills and peaked career</label>
            <input type="number" min="0" class="form-control" oninput="topbillingChange(value)" id="topbilling" value="<?= (trim($s2legacy['topbilling'])) ?>" name="topbilling"/>
        </div>
    </div>
</div>
<input type="hidden" name="petstotal" id="pt" value="<?= (trim($s2legacy['petstotal'])) ?>">
<p class="lead"><b>Sub total: <output id="petstotal" for="pt"><span
    <?php if ($s2legacy['petstotal']  == 0): ?>class="badge badge-primary"<?php endif; ?>
    <?php if ($s2legacy['petstotal']  > 0): ?>class="badge badge-success"<?php endif; ?>
    <?php if ($s2legacy['petstotal']  < 0): ?>class="badge badge-danger"<?php endif; ?>>
    <?= (trim($s2legacy['petstotal']))."
" ?>
	</span></output></b></p>
...
<script>
	let f = 0;
	$(function () {
		$('#casfounder').val(<?= ($s2legacy['casfounder']) ?>);
		petsCheck();
	});

	function petsCheck() {
		if ( $('#casfounder').prop("checked"))  {
			if (<?= ($s2legacy['casfounder']) ?> == 0) {
				f += 0.5;
			}
			else
				f = 0.5;
		} else {
			if (<?= ($s2legacy['casfounder']) ?> == 1) {
				f = 0.5;
				f -= 0.5;
			}
		else
