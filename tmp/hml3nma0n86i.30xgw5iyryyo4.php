<h3 class="text-primary"><?= ($cat) ?> <a href="#" class="badge badge-pill badge-secondary title" data-toggle="collapse" data-target="#moneyHelp" aria-expanded="false" aria-controls="moneyHelp">
    ?</a>
    </a></h3>
    ...
    <div class="col">
        <div class="form-group">
            ...
            <div class="input-group mb-3">
                ...
                <input type="number" id="money" min="0" name="money" value="<?= (trim($household['money'])) ?>" oninput="moneyChange(value)" class="form-control" aria-label="money" aria-describedby="money">
            </div>
        </div>
    </div>
</div>
<input type="hidden" name="moneytotal" id="mt" value="<?= (trim($s2legacy['moneytotal'])) ?>">
    <p class="lead mt-3"><b>Sub total: <output id="moneytotal" for="mt"><span
        <?php if ($s2legacy['moneytotal']  == 0): ?>class="badge badge-primary"<?php endif; ?>
        <?php if ($s2legacy['moneytotal']  > 0): ?>class="badge badge-success"<?php endif; ?>
        <?php if ($s2legacy['moneytotal']  < 0): ?>class="badge badge-danger"<?php endif; ?>>
        <?= (trim($s2legacy['moneytotal']))."
" ?>
        </span></output></b></p>
