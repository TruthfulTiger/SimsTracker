<h3 class="text-primary"><?= ($cat) ?> <img src="<?= ($BASE) ?><?= ($s2path) ?>OFB.png" alt="Needs OFB"> <a href="#" class="badge badge-pill badge-secondary title" data-toggle="collapse" data-target="#bizHelp" aria-expanded="false" aria-controls="bizHelp">
    ?</a>
    </a></h3>
...
<div class="form-row">
    <div class="col">
        <div class="form-group">
            ...
            <input type="number" min="0" class="form-control" oninput="gensChange(value)" value="<?= (trim($s2legacy['busgens'])) ?>" id="busgens" name="busgens"/>
        </div>
        <div class="form-group">
            <label for="badgecount">Sims with all 7 Gold Talent Badges</label>
            <input type="number" min="0" class="form-control" oninput="badgeChange(value)" value="<?= (trim($s2legacy['badgecount'])) ?>" id="badgecount" name="badgecount"/>
        </div>
    </div>
</div>
<input type="hidden" name="biztotal" id="bt" value="<?= (trim($s2legacy['biztotal'])) ?>">
<p class="lead mt-3"><b>Sub total: <output id="biztotal" for="bt"><span
    <?php if ($s2legacy['biztotal']  == 0): ?>class="badge badge-primary"<?php endif; ?>
    <?php if ($s2legacy['biztotal']  > 0): ?>class="badge badge-success"<?php endif; ?>
    <?php if ($s2legacy['biztotal']  < 0): ?>class="badge badge-danger"<?php endif; ?>>
