<h3 class="text-primary"><?= ($cat) ?> <a href="#" class="badge badge-pill badge-secondary title" data-toggle="collapse" data-target="#friendsHelp" aria-expanded="false" aria-controls="friendsHelp">
    ?</a>
    </a></h3>
...
<div class="form-row">
    <div class="col">
        <div class="form-group">
            ...
            <input type="number" min="0" id="friends" name="friends" value="<?= (trim($household['friends'])) ?>" oninput="friendsChange(value)" class="form-control" aria-label="friends" aria-describedby="friends">
        </div>
    </div>
</div>
<input type="hidden" name="friendstotal" id="ft" value="<?= (trim($s2legacy['friendstotal'])) ?>">
<p class="lead mt-3"><b>Sub total: <output id="friendstotal" for="ft"><span
    <?php if ($s2legacy['friendstotal']  == 0): ?>class="badge badge-primary"<?php endif; ?>
    <?php if ($s2legacy['friendstotal']  > 0): ?>class="badge badge-success"<?php endif; ?>
    <?php if ($s2legacy['friendstotal']  < 0): ?>class="badge badge-danger"<?php endif; ?>>
