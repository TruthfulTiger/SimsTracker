<h3 class="text-primary"><?= ($cat) ?> <img src="<?= ($BASE) ?><?= ($s2path) ?>FT.png" alt="Needs FT"> <a href="#" class="badge badge-pill badge-secondary title" data-toggle="collapse" data-target="#ftHelp" aria-expanded="false" aria-controls="ftHelp">
	?</a>
	</a></h3>
        ...
        <div class="form-group">
            ...
            <select class="custom-select" name="familyhobby" id="familyhobby">
                <?php foreach (($hobbies?:[]) as $key=>$hobby): ?>
					...
					selected
                     <?php endif; ?>><?= (trim($hobby)) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="form-group">
            <label for="fhmax">Number of Sims to max Family Hobby</label>
            <input type="number" min="0" class="form-control" id="fhmax" oninput="ftmaxChange(value)" value="<?= (trim($s2legacy['fhmax'])) ?>" name="fhmax"/>
        </div>
        <div class="form-group">
            <div class="custom-control custom-checkbox">
				<input type="checkbox" class="custom-control-input" value="1" onchange="ftCheck()" name="pdhobby" id="pdhobby"
				<?php if ($s2legacy['pdhobby'] == 1): ?>
					checked
				<?php endif; ?>
				/>
...
<p class="lead mb-3"><b>Sub total: <output id="fttotal" for="ftt"><span
    <?php if ($s2legacy['fttotal']  == 0): ?>class="badge badge-primary"<?php endif; ?>
    <?php if ($s2legacy['fttotal']  > 0): ?>class="badge badge-success"<?php endif; ?>
    <?php if ($s2legacy['fttotal']  < 0): ?>class="badge badge-danger"<?php endif; ?>>
...
<script>
	...
	function ftCheck() {
		if ( $('#pdhobby').prop("checked"))  {
			if (<?= ($s2legacy['pdhobby']) ?> == 0) {
				ft ++;
			}
		else
			ft = 1;
		} else {
			if (<?= ($s2legacy['pdhobby']) ?> == 1) {
				ft = 1;
				ft --;
			}
		else
